...
def _stem(word: str) -> str:
    """Cheap stem: lowercase, first 4 chars — so 'price'/'pricing'/'priced' share the stem
    'pric', while staying specific enough that 'pricing' never matches 'lunch'."""
    return word.lower()[:4]
...
def _collect_valued(evidence: Sequence[Any], kws: set[str]) -> list[tuple[Any, list[tuple[str, str]]]]:
    ...
    """
    kw_stems = {_stem(k) for k in kws}
    out: list[tuple[Any, list[tuple[str, str]]]] = []
    for ev in evidence:
        text = getattr(ev, "text", "") or ""
        if kw_stems and not (kw_stems & _text_stems(text)):
            continue
